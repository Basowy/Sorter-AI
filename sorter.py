...
SUPPORTED_EXTS = (".epub", ".pdf", ".txt", ".mobi", ".azw3", ".html", ".htm", ".rtf", ".zip", ".rar", ".doc")
...
def classify_with_ollama(title, author, snippet):
    # Zaimplementuj tutaj logikę komunikacji z modelem OLLAMA
    # Przykład:
    print(f"Sending to OLLAMA: title={title}, author={author}, snippet={snippet}")
    genre = "Unknown"  # Zamiennik dla odpowiedzi OLLAMA
    confidence = 0.7
    explanation = "Classification generated by AI"
...
def extract_rar_to_temp(rar_path: str, tmpdir: str) -> str:
    """
    Rozpakowuje RAR do tmpdir.
    Zwraca ścieżkę folderu z wypakowaną zawartością.
    """
    if not os.path.exists(SEVEN_ZIP_PATH):
        raise RuntimeError(f"Nie znaleziono 7z.exe pod: {SEVEN_ZIP_PATH}")

    out_dir = os.path.join(tmpdir, "rar_extract")
    ...
    if r.returncode != 0:
        raise RuntimeError(f"Błąd rozpakowywania RAR:\n{r.stderr}\n{r.stdout}")

    return out_dir


def extract_text_from_doc(path: str, max_chars=8000) -> str:
    """
    DOC -> tekst przy pomocy 7-Zip:
    - 7z potrafi wypakować DOC jako OLE storage
    - potem próbujemy wyciągnąć "WordDocument" jako binarny tekst (fallback)
    
    UWAGA: DOC jest starym formatem binarnym — to nie będzie idealne,
    ale w praktyce daje sporo tekstu do LLM.
    """
    try:
        if not os.path.exists(SEVEN_ZIP_PATH):
            return ""
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            out_dir = os.path.join(tmpdir, "doc_extract")
            os.makedirs(out_dir, exist_ok=True)
            ...
            # w DOC najczęściej jest plik WordDocument
            worddoc_path = None
            for root, dirs, files in os.walk(out_dir):
                for fn in files:
                    if fn.lower() == "worddocument":
                        worddoc_path = os.path.join(root, fn)
                        break
                if worddoc_path:
                    break
            ...
            # czytamy binarnie i wyciągamy "czytelne" fragmenty
            with open(worddoc_path, "rb") as f:
                data = f.read()

            # wyciągamy ciągi ascii/latin/cp1250 jako tekst
            # (brutalne, ale działa jako fallback)
            try:
                text = data.decode("cp1250", errors="ignore")
            except:
                text = data.decode("latin-1", errors="ignore")

            # usuń dużo śmieci binarnych
            text = re.sub(r"[\x00-\x08\x0B\x0C\x0E-\x1F]", " ", text)
            text = clean_text(text)

            # odetnij, bo doc potrafi być gigantyczny
            return text[:max_chars]
    ...
    for root, dirs, filenames in os.walk(INPUT_DIR):
        for fn in filenames:
            ...
            if ext == ".rar":
                try:
                    with tempfile.TemporaryDirectory() as tmpdir:
                        extracted_dir = extract_rar_to_temp(full_path, tmpdir)

                        # szukamy plików książek wewnątrz
                        for rr, dd, ff in os.walk(extracted_dir):
                            for inner_fn in ff:
                                inner_ext = os.path.splitext(inner_fn)[1].lower().strip()

                                if inner_ext in SUPPORTED_EXT:
                                    inner_full = os.path.join(rr, inner_fn)

                                    # traktujemy jako "file", ale oznaczamy że pochodzi z RAR
                                    items.append(("rar_file", full_path, inner_full))
        ...
        try:
            ...
            if item[0] == "file":
                ...
                if ext == ".epub":
                    ...
                    title = os.path.splitext(os.path.basename(path))[0]
                    snippet = extract_text_from_doc(path, max_chars)
                else:
                    title = os.path.splitext(os.path.basename(path))[0]
                    snippet = ""
            ...
            # B1.5) Plik z RAR
            # ==========================================
            elif item[0] == "rar_file":
                rar_path = item[1]
                extracted_path = item[2]
                ext = os.path.splitext(extracted_path)[1].lower()

                title, author, snippet = "", "", ""

                if ext == ".epub":
                    title, author, snippet = extract_from_epub(extracted_path, max_chars)
                elif ext == ".pdf":
                    title, author, snippet = extract_from_pdf(extracted_path, max_chars)
                elif ext in (".html", ".htm"):
                    title = os.path.splitext(os.path.basename(extracted_path))[0]
                    snippet = extract_text_from_html(extracted_path, max_chars)
                elif ext in (".txt", ".rtf"):
                    title = os.path.splitext(os.path.basename(extracted_path))[0]
                    snippet = extract_from_txt_like(extracted_path, max_chars)[2]
                elif ext == ".doc":
                    title = os.path.splitext(os.path.basename(extracted_path))[0]
                    snippet = extract_text_from_doc(extracted_path, max_chars)
                else:
                    title = os.path.splitext(os.path.basename(extracted_path))[0]
                    snippet = ""
                ...
                # fallback autora
                if not author.strip():
                    author = guess_author_from_filename(extracted_path)
                if not author.strip():
                    author = extract_author_with_llm(model, title, snippet, max_chars=max_chars)
                ...
                # kopiujemy plik rozpakowany z rar
                dest = os.path.join(out_dir, os.path.basename(extracted_path))
                if not os.path.exists(dest):
                    shutil.copy2(extracted_path, dest)

                rows.append({
                    "file": f"{rar_path}::{extracted_path}",
                    "title": title,
                    "author": author,
                    "genre": genre,
                    "format": fmt,
